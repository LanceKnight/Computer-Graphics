...
std::string
TiffRead::tiff_read(std::vector<std::string> paramList){
	std::string result;
	char buffer[3];
	short magicno;
	int ifdaddress;
...
void
TiffRead::IFD_intepret(unsigned char* IFD, bool should_reverse,  std::ifstream & file){
...
std::string 
TiffRead::type_intepret(short code){
	switch(code){
		case 1:
			return "Byte";
		case 2:
...
int
TiffRead::type_length_intepret(short code){
	switch(code){
		case 1:
			return 1;
		case 2:
...
void
TiffRead::type_output_intepret(short code, unsigned char *data_array, int n, bool should_reverse){
